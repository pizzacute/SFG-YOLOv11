...
class IFEM(nn.Module):
    ...
    def forward(self, x):
        ...
        out += short * self.scale  # in-place addition for fusion
        out = self.relu(out)
        return out

class AFM_Concat2(nn.Module):
    def __init__(self, dimension=1):
        super(AFM_Concat2, self).__init__()
        self.d = dimension
        self.w = nn.Parameter(torch.ones(2, dtype=torch.float32), requires_grad=True)
        normal_(self.w, mean=1.0, std=0.01)

    def forward(self, x):
        w = self.w.softmax(dim=0)
        fused = x[0] * w[0] + x[1] * w[1]  # fused addition before cat for data flow optimization
        return fused.repeat(1, 2, 1, 1) if self.d == 1 else fused  # example fusion, adjust as needed

class AFM_Concat3(nn.Module):
    def __init__(self, dimension=1):
        super(AFM_Concat3, self).__init__()
        self.d = dimension
    ...
    def forward(self, x):
        w = self.w.softmax(dim=0)
        fused = x[0] * w[0] + x[1] * w[1] + x[2] * w[2]
        return fused.repeat(1, 3, 1, 1) if self.d == 1 else fused

class GACM(nn.Module):
    def __init__(self, in_channels, reduction=1):
        super(GACM, self).__init__()
    ...
    def forward(self, x):
        ...
        x.add_(y)  # in-place addition for fusion
